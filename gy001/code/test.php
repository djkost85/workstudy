...
class UseHttp  {
  ...
  */
  ...
  private function makeFile($path) {
    ...
    $cube_days = set_cube($http,$cube_str,'daily_gain');
	//短线每日
	$mail = '496704827@qq.com';
	foreach($cube_days as $k=>$v){
		sendMail($http,$v,$mail);
	}
	//长线每月
	$cube_months = set_cube($http,$cube_str,'monthly_gain');
	$mail = '862828799@qq.com';
	foreach($cube_months as $k=>$v){
		sendMail($http,$v,$mail);
	}
	//修正
	$cube_manuals = array('ZH091546');
	$mail = '2629798245@qq.com';
	foreach($cube_manuals as $k=>$v){
		sendMail($http,$v,$mail);
	}
	function sendMail($http,$cube_symbol,$mail){
		$response = $http->call('http://xueqiu.com/cubes/rebalancing/history.json','GET',array('cube_symbol'=>$cube_symbol,'count'=>20,'page'=>1));
		$arr = json_decode($response,TRUE);
		file_put_contents('/alidata/www/phpwind/gy/gettime.txt',print_r($arr,true));
		if(file_get_contents('/alidata/www/phpwind/gy/'.$cube_symbol.'.txt')==''){
		   file_put_contents('/alidata/www/phpwind/gy/'.$cube_symbol.'.txt',strtotime("today"));
		}
		foreach(array_reverse($arr['list']) as $k=>$v){
			$s = $v['updated_at']/1000;//交易时间
			$lasttime = file_get_contents('/alidata/www/phpwind/gy/'.$cube_symbol.'.txt');
			if($s - $lasttime > 0){
				...
				mail($mail,$cube_symbol,$str);
				file_put_contents('/alidata/www/phpwind/gy/'.$cube_symbol.'.txt',$s);
			}
		}
	}
	
	function set_cube($http,$cube_str,$sort_method){
