...
ini_set('display_errors', 1);
    ...
    $http->cookieSave = 0;
	function save_data($http,$j){
		for($i=$j;$i<$j+500;$i++){
			$m = ($i<10000)?'ZH00':'ZH0';
			$response = $http->call('http://xueqiu.com/cubes/quote.json','GET',array('code'=>$m.$i));
			$arr = json_decode($response,TRUE);
			foreach($arr as $k=>$v){
				if(isset($v['market'])&&$v['market']=='cn'&&$v['total_gain']>70&&$v['monthly_gain']>5){
					file_put_contents('/alidata/www/phpwind/gy/cube_anuals/all_cube_total_'.$j.'.txt',$k.'|',FILE_APPEND);
				}			
			}
		}
	...
	function do_save($http){
		for($i=0;$i<=23;$i++){
			$j = ($i<=9)?'0'.$i:$i;
			if(date('H-i')==$j.'-00'){
				save_data($http,1000*$i+1000);
			}
			if(date('H-i')==$j.'-06'){
				save_data($http,1000*$i+1500);
			}
			if(date('H-i')==$j.'-12'){
				save_data($http,1000*$i+2000);
			}
			if(date('H-i')==$j.'-18'){
				save_data($http,1000*$i+2500);
			}
			if(date('H-i')==$j.'-24'){
				save_data($http,1000*$i+2000);
			}
