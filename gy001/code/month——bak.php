...
include('/alidata/www/phpwind/gy/curl.class.php');
	function save_data($j){
		for($i=$j;$i<$j+500;$i++){			
			...
			foreach($arr as $k=>$v){
				if(isset($v['market'])&&$v['market']=='cn'&&$v['total_gain']>0&&$v['monthly_gain']>7){
					file_put_contents('/alidata/www/phpwind/gy/cube_anuals/all_cube_monthly_gain.txt',$k.'|',FILE_APPEND);
				}
				if(isset($v['market'])&&$v['market']=='cn'&&$v['total_gain']>75&&$v['monthly_gain']>0){
					file_put_contents('/alidata/www/phpwind/gy/cube_anuals/all_cube_total_gain.txt',$k.'|',FILE_APPEND);
				}else{
